...
u8 run_target(char** argv, u32 timeout) {
  ...
  ++total_execs;
        ...
        fprintf(logfp, "Length:%lu Cumulative:%lu", *(input_stat_ptr->tracelength_ptr), trace_cumulative);
        fclose(logfp);
      #endif
     trace_cumulative += *(input_stat_ptr->tracelength_ptr); 
     // Reset the trace length counter after one run of the target
     *(input_stat_ptr->tracelength_ptr) = 0; 
   #endif
  #endif
