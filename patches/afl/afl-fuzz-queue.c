...
void add_to_queue(u8* fname, u32 len, u8 passed_det) {
  ...
  #ifdef AF
    #ifdef TRACE_METRIC
      q->trace_length = *(input_stat_ptr->tracelength_ptr); 
    #endif
  #endif
  q->len = len;
  q->depth = cur_depth + 1;
...
u32 calculate_score(struct queue_entry* q) {
  ...
  #ifdef AF
    #ifdef TRACE_METRIC
      u64 avg_trace_length = trace_cumulative / total_execs;
    #endif
  #endif
  u32 perf_score = 100;
  ...
  //
  // AF: We reverse the perf_score assignment by giving slower inputs more airtime
  // The reasoning is that slower inputs are spending more time in the area cordoned
  // off by us and hence are more likely to reach our target of interest.
  #ifdef AF
  ...
  if (q->exec_us * 0.1 > avg_exec_us)
    ...
    perf_score = 150;
  #ifdef TRACE_METRIC
    if (q->trace_length * 0.3 > avg_trace_length)
      perf_score *= 3;
    else if (q->trace_length * 0.5 > avg_trace_length)
      perf_score *= 2;
    else if (q->bitmap_size * 0.75 > avg_trace_length)
      perf_score *= 1.5;
    else if (q->trace_length * 3 < avg_trace_length)
      perf_score *= 0.25;
    else if (q->trace_length * 2 < avg_trace_length)
      perf_score *= 0.5;
    else if (q->trace_length * 1.5 < avg_trace_length)
      perf_score *= 0.75;
  #endif
